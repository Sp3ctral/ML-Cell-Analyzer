...
class App:
    ...
    def __init__(self):
        ...
        layout1 = [[sg.Menu(menu_def)],
                   [sg.Text('Select Video or Image:', font=TITLE_FONT, text_color=TITLE_COLOR)],
                   [sg.Input(key="_FILEPATH_"), sg.Button("Browse")],  # File Selector
                   ...
                   [sg.Text('Select Type of Cell Tracking:', font=TITLE_FONT, text_color=TITLE_COLOR), sg.Push(),
                    sg.Text('Tracker Settings. Required*', font=TITLE_FONT, text_color=TITLE_COLOR)],
                   ...
                   [sg.R('Individual Cell Tracking', 1, key="individual_radio", background_color=BACKGROUND_COLOR),
                    sg.Push(), sg.Text('Real World Width of Footage (mm)*'), sg.Input(key="video_width_mm")],
                   # sg.Push(), sg.Text('Number of pixels per mm* (If height/width are unknown)'), sg.Input(key='pixels_per_mm')],
                   # Take Input for Constants
                   [sg.R('Full Culture Tracking', 1, key="culture_radio", background_color=BACKGROUND_COLOR), sg.Push(),
                    sg.Text('Real World Height of the Footage (mm)*'), sg.Input(key="video_height_mm")],
                   ...
                   [sg.R('Spheroid Tracking', 1, key="spheroid_radio", background_color=BACKGROUND_COLOR), sg.Push(),
                    sg.Text('Number of pixels per mm* (If height/width are unknown)'), sg.Input(key='pixels_per_mm')],
                   ...
                   [sg.Text('Select Units for Exported Data:', font=TITLE_FONT, text_color=TITLE_COLOR), sg.Push(),
                    sg.Text('Time Between Images (mins)* (If video type)'), sg.Input(key="time_between_frames")],
                   [sg.R('µm', 3, key="units_µm", default=True, background_color=BACKGROUND_COLOR), sg.Push(),
                    sg.Text('Film Editing', font=TITLE_FONT, text_color=TITLE_COLOR)],
                   ...
                   [sg.R('mm', 3, key="units_mm", background_color=BACKGROUND_COLOR), sg.Push(),
                    sg.Text('Min Cell Perimeter (Default = 10 pixels)'), sg.Input(key="min_size")],
                   [sg.Push(), sg.Text('Background Hex Code (E.x: #663399)'), sg.Input(key="hex_code")],
                   # sg.Push(), sg.Text('Contrast (Default = 1.25)'), sg.Input(key="contrast")],
                   [sg.Text('Select Video Playback Speed:', font=TITLE_FONT, text_color=TITLE_COLOR), sg.Push(),
                    sg.Text('Max Cell Perimeter (Default = 600 pixels)'), sg.Input(key="max_size")],
                   [sg.R('1x', 2, key="playback_radio_1x", default=True, background_color=BACKGROUND_COLOR),
                    ...
                    sg.R('50x', 2, key="playback_radio_50x", background_color=BACKGROUND_COLOR), sg.Push(),
        ...
        layout2 = [[sg.Menu(menu_def)],
                   ...
                   [sg.Button("Pause", key="Play"),
                    # Add here if the next frame button is desired sg.Button('Next frame')
                    sg.Push(), sg.Button('Export Data', key='Export Data', disabled=True),
        ...
        # Spheroid Selection (For Spheroid Tracking)
        layout4 = [[sg.Menu(menu_def)],
                   ...
                   # # Windows for edited/original video to play
                   # [sg.Canvas(size=(400, 300), key="original_first_frame_s", background_color="blue", visible=False),
                   # sg.Canvas(size=(400, 300), key="edited_first_frame_s", background_color="blue", visible=False)],

                   # # Take input of Cell ID Number
                   # [sg.Text('Enter Id number of cell you wish to track:'), sg.Input(key="cell_id", enable_events=True)],

                   # Write instructions
                   [sg.Text('Select Spheroid to track', font=TITLE_FONT)],
                   [sg.Text('On the pop up window, click and drag across the length of the desired spheroid')],
                   [sg.Text('Repeat until satisfied, then click the spacebar to close window')],
                   [sg.Text('Click track to proceed')],

                   # Run and Exit Buttons
                   [sg.Button('Track', key="track_spheroid"), sg.Button('Restart'), sg.Button("Exit")]]

        # Export Data Menu
        layout5 = [[sg.Menu(menu_def)],
                   [sg.Text("Select Export Settings", font=TITLE_FONT, text_color=TITLE_COLOR)],
                   [sg.Text('Select Directory to Export to:'), sg.Input(key="export_directory"),
                    sg.Button("Browse", key="export_browse")],  # Directory Selector

                   [sg.Check('Export Data to Excel Sheet', key='excel_export', enable_events=True,
                             background_color=BACKGROUND_COLOR)],
                   [sg.Text(
                       'Excel File to Export to (.xlsx):\n  Leave blank for auto generated export to Downloads folder',
                       key='excel_file_label', visible=False), sg.Input(key="excel_filename", visible=False),
                       sg.Text(".xlsx", key="excel_ext", visible=False)],
                   [sg.Check('Export Raw Data to CSV File', key='csv_export', enable_events=True,
                             background_color=BACKGROUND_COLOR)],
                   [sg.Text(
                       'CSV File to Export to (.csv):\n  Leave blank for auto generated export to Downloads folder',
                       key='csv_file_label', visible=False), sg.Input(key="csv_filename", visible=False),
                       sg.Text(".csv", key="csv_ext", visible=False)],
                   [sg.Text('Select Graphs to Export:', key="Graph Title", visible=False, font=TITLE_FONT,
                            text_color=TITLE_COLOR)],

                   [sg.Check('Area over Time', key="Area over Time", enable_events=True, visible=False,
                             background_color=BACKGROUND_COLOR)],
                   [sg.Text(
                       ...
                       sg.Text(".pdf", key="area_graph_ext", visible=False)],
                   ...
                   [sg.Check('Movement over Time', key='Movement over Time', enable_events=True, visible=False,
                             background_color=BACKGROUND_COLOR)],
                   [sg.Text(
                       'Number of Points to Label.\n  By default only the First and Last point will be labeled, reducing this number improves visual clarity',
                       key="num_labels_desc", visible=False), sg.Input(key='num_labels', visible=False)],
                   [sg.Text(
                       'Filename to save graph to (.pdf):\n  Leave blank for customizable settings and manual save after graph creation',
                       key='individual_movement_graph_label', visible=False),
                       sg.Input(key="individual_movement_graph_filename", visible=False),
                       sg.Text(".pdf", key="individual_movement_graph_ext", visible=False)],

                   [sg.Text('Select Images to Export', key="images_label", visible=False, font=TITLE_FONT,
                            text_color=TITLE_COLOR)],
                   [sg.Check('Export Final Path of Tracked Cell', key="path_image", visible=False, enable_events=True,
                             background_color=BACKGROUND_COLOR)],
                   [sg.Text(
                       'Filename to save image to (.png):\n  Leave blank for autogenerated export to Downloads folder',
                       key='final_path_image_label', visible=False),
                       sg.Input(key="final_path_image_filename", visible=False),
                       sg.Text(".png", key="final_path_image_ext", visible=False)],

                   # Spheroid Tracker Specific Export Items
                   [sg.Check('Movement over Time', key='Movement over Time', enable_events=True, visible=False,
                             background_color=BACKGROUND_COLOR)],
                   [sg.Text(
                       'Number of Points to Label.\n  By default only the First and Last point will be labeled, reducing this number improves visual clarity',
                   ...
                   [sg.Text(
                       'Filename to save graph to (.pdf):\n  Leave blank for customizable settings and manual save after graph creation',
                       key='spheroid_movement_graph_label', visible=False),
                       sg.Input(key="spheroid_movement_graph_filename", visible=False),
                       sg.Text(".pdf", key="spheroid_movement_graph_ext", visible=False)],

                   [sg.Text('Select Images to Export', key="images_label", visible=False, font=TITLE_FONT,
                            text_color=TITLE_COLOR)],
                   [sg.Check('Export Final Path of Tracked Spheroid', key="path_image", visible=False,
                             enable_events=True,
                             background_color=BACKGROUND_COLOR)],
                   [sg.Text(
                       'Filename to save image to (.png):\n  Leave blank for autogenerated export to Downloads folder',
                   ...
                   [sg.Check('Average Displacement', key="average_displacement", visible=False, enable_events=True,
                             background_color=BACKGROUND_COLOR)],
                   [sg.Text(
                       'Filename to save graph to (.pdf):\n  Leave blank for customizable settings and manual save after graph creation',
                       key='culture_displacement_graph_label', visible=False),
                       sg.Input(key="culture_displacement_graph_filename", visible=False),
                       sg.Text(".pdf", key="culture_displacement_graph_ext", visible=False)],

                   [sg.Check('Average Speed', key="average_speed", visible=False, enable_events=True,
                             background_color=BACKGROUND_COLOR)],
                   [sg.Text(
                       'Filename to save graph to (.pdf):\n  Leave blank for customizable settings and manual save after graph creation',
                       key='culture_speed_graph_label', visible=False),
                       sg.Input(key="culture_speed_graph_filename", visible=False),
                       sg.Text(".pdf", key="culture_speed_graph_ext", visible=False)],
        ...
        layout6 = [[sg.Menu(menu_def)],
                   [sg.Text("Export Successful", key="title", font="Times 16", text_color=TITLE_COLOR,
                            justification='c')],
        ...
        layout = [[sg.Image(source="bruin.png", size=(85, 55), subsample=29, background_color=BACKGROUND_COLOR),
                   sg.Text("Cell Analyzer", key="title", font="Times 18 bold italic", text_color=TITLE_COLOR,
                           justification='c')],  # program title and logo image
                  [sg.Column(layout1, key='-COL1-', scrollable=True, size=(screen_width, screen_height)),
                   sg.Column(layout2, visible=False, key='-COL2-', scrollable=True, size=(screen_width, screen_height)),
                   sg.Column(layout3, visible=False, key='-COL3-', scrollable=True, size=(screen_width, screen_height)),
                   sg.Column(layout4, visible=False, key='-COL4-', scrollable=True, size=(screen_width, screen_height)),
                   sg.Column(layout5, visible=False, key='-COL5-', scrollable=True, size=(screen_width, screen_height)),
                   sg.Column(layout6, visible=False, key='-COL6-', scrollable=True,
        ...
        self.window = sg.Window('Cell Analyzer', layout, resizable=True, size=(screen_width, screen_height),
        ...
        # Uncomment to display frames in spheroid selection page
        # self.first_frame_orig_s = self.window.Element("original_first_frame_s").TKCanvas
        # self.first_frame_edited_s = self.window.Element("edited_first_frame_s").TKCanvas
        ...
        while running:
            ...
            if event == "Run":
                ...
                if self.window["hex_code"].get():
                    background_hex = self.window["hex_code"].get()
                ...
                if isValidParameters(file, width, height, mins, pixels_per_mm, min_size, max_size, contrast, brightness,
                                     blur):
                    ...
                    # If spheroid tracking has been selected
                        ...
                        # Draw circle here
                        self.video_player.select_spheroid()  # deals with first frame
            ...
            if event == "track_spheroid":
                # When Track button is pressed update the spheroid tracker to keep track of the input cell
                # and then attempt to move forward to video player stage

                print("ready to track spheroid")
                # TODO: draw circle here? NO! spheroid should already be selected and processed
                # TrackerClasses.SpheroidTracker.select_spheroid(self)

                # Set the tracker's cell id to 0, because after processing there should only be one object
                self.video_player.set_tracked_cell(0)
                ...
                if self.frames == 1:
                    self.window[f'-COL{SPHEROID_SELECTION}-'].update(visible=False)
            ...
            if event == "cell_id":
                ...
                # If the entered value is a valid cell id
                    ...
                    outlined_cell_img = PIL.ImageTk.PhotoImage(
                        image=PIL.Image.fromarray(selected_cell_img).resize((self.vid_width, self.vid_height),
                                                                            Image.NEAREST)
                    )
            ...
            if event == "Export":
                ...
                if valid_filenames and area_graph_filename != '' and area_graph_filename is not None:
                    ...
                    if not isValidGraphFilename(area_graph_filename):
                        valid_filenames = False
                        sg.PopupError(
                            "Given Area over Time Graph's File Name is in an incorrect format.\nEnsure the filename ends in "
                    ...
                    if exportgraph_area:
                        ...
                        if area_graph_filename != '' and area_graph_filename is not None:
                            self.video_player.export_area_graph(
                                filename=f"{export_directory}/{area_graph_filename}.pdf")
                        else:
                            self.video_player.export_area_graph()
                            ...
                            if num_labels != '' and num_labels is not None:
                                ...
                                if individual_movement_graph_filename != '' and individual_movement_graph_filename is not None:
                                    self.video_player.export_movement_graph(num_labels=int(num_labels),
                                                                            filename=f"{export_directory}/{individual_movement_graph_filename}.pdf")
                                else:
                                    self.video_player.export_movement_graph(num_labels=int(num_labels))
                            else:
                                # If a filename was supplied pass it as a parameter
                        ...
                        if exportpath_image:  # and self.window.Element("individual_radio").get():
                            ...
                            if path_image_filename != '' and path_image_filename is not None:
                                self.video_player.export_final_path(
                                    filename=f"{export_directory}/{path_image_filename}.png")
                            else:
                                self.video_player.export_final_path()
                        ...
                        if export_average_displacement:
                            ...
                            if culture_displacement_graph_filename != '' and culture_displacement_graph_filename is not None:
                                self.video_player.export_average_displacement_graph(
                                    filename=f"{export_directory}/{culture_displacement_graph_filename}.pdf")
                            else:
                                self.video_player.export_average_displacement_graph()
                        ...
                        if export_average_speed:
                            ...
                            if culture_speed_graph_filename != '' and culture_speed_graph_filename is not None:
                                self.video_player.export_average_speed_graph(
                                    filename=f"{export_directory}/{culture_speed_graph_filename}.pdf")
                            else:
                                self.video_player.export_average_speed_graph()
            ...
            if event.startswith("Restart"):
                ...
                fields_to_clear = ["_FILEPATH_", "video_width_mm", "video_height_mm", "pixels_per_mm",
                                   "time_between_frames",
                                   "min_size", "max_size", "contrast", "brightness", "blur", "cell_id",
                                   "excel_filename", "csv_filename",
                                   "area_graph_filename", "individual_movement_graph_filename",
    ...
    def update(self):
        ...
        if self.vid:
            ...
            print("VIDEO_PLAYER.visible:", self.window[f'-COL{VIDEO_PLAYER}-'].visible)
    ...
    def display_first_frame(self):
        ...
        print("made it to display_first_frame")
        unedited, processed = self.video_player.get_first_frame()
        # cv2.imshow("unedited", unedited)
        ...
        # self.first_frame_orig_s.config(width=self.vid_width, height=self.vid_height)
        # self.first_frame_edited_s.config(width=self.vid_width, height=self.vid_height)
        #
        #
        # # Update right side of counter
        ...
        # self.first_frame_orig_s.create_image(0, 0, image=self.photo, anchor=tk.NW)
        #
        ...
        # self.first_frame_edited.create_image(0, 0, image=self.edited, anchor=tk.NW)
        # self.first_frame_edited_s.create_image(0, 0, image=self.edited, anchor=tk.NW)
        # print("created images for windows")
        # # Make canvas elements visible
        ...
        # self.window.refresh()
        # print("refreshed")
        if self.window[f'-COL{SPHEROID_SELECTION}-'].visible:
            # change canvas size approx to video size
            # self.first_frame_orig_s.config(width=self.vid_width, height=self.vid_height)
            # self.first_frame_edited_s.config(width=self.vid_width, height=self.vid_height)
            ...
            #
            # # Make canvas elements visible
            # self.window['original_first_frame_s'].update(visible=True)
            # self.window['edited_first_frame_s'].update(visible=True)
            # Update Scroll Bar to fit contents
            self.window.refresh()
            # Update window
...
def isValidParameters(videofile, width, height, time_between_frames, pixels, min_size, max_size, contrast, brightness,
                      blur):
    valid = False
    ...
    if isValidVideo(videofile):
        ...
        if isValidTime(time_between_frames):
                ...
                if (isValidInt(min_size) and (min_size != "" and min_size is not None)) or min_size == "":
                    ...
                    if (isValidInt(max_size) and (max_size != "" and max_size is not None)) or max_size == "":
                        ...
                        if (isValidFloat(contrast) and (contrast != "" and contrast is not None)) or contrast == "":
                            # Validate brightness
                            if (isValidFloat(brightness) and (
                                    brightness != "" and brightness is not None)) or brightness == "":
                                # Validate Blur
                                ...
                                # Display Blur Error Messsage
                                    ...
                                    sg.popup_error(
                                        "Entered: Blur intensity is invalid. Blur intensity must be a positive integer or left empty for the default value")
            ...
            # Dimensions/Pixel Measurement Error
                ...
                sg.popup_error(
                    "Entered: Dimensions or Pixels per mm is invalid. Either the width and height fields or the pixels per mm field must be filled. They must be a positive integer/float")
        # Time Error Message
        else:
            sg.popup_error(
